...
def completeflippingprogram(S,K):
    ...
    while islandcounter(S)>1 and len(S[0][findP(S):])>=K:
        S=minuscongeal(S,K)
    if islandcounter(S)==1:
        lengthisland=len(S[0][findP(S):lastpos(S)])
        if lengthisland%K==0:
            S[0]=S[0][:findP(S)]+('+'*lengthisland)+S[0][lastpos(S):]
            S[1]=S[1]+lengthisland//K
    if checkforminus(S)==False:
        return 'IMPOSSIBLE'
    return S
print(completeflippingprogram(['++--------------------+++++--------------------++--------------------+--------------------+--------------------+--------------------++----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------',0],20))
...
for line in infile:
    ...
    allexpts.append(listline)
print(allexpts)
infile.close()
outfile=open('googletestlargetry3.out','w')
for n in range(len(allexpts)):
    testsubject=completeflippingprogram([allexpts[n][0],0],allexpts[n][1])
    print(testsubject)
    writeimp=''
    writenum=''
    if testsubject=='IMPOSSIBLE':
        writeimp='Case #'+str(n+1)+': '+str(testsubject)
        print(writeimp)
        outfile.write(writeimp+'\n')
    else:
        numflips=testsubject[1]
        writenum='Case #'+str(n+1)+': '+str(numflips)
        print(writenum)
        outfile.write(writenum+'\n')
outfile.close()
print(completeflippingprogram(['-+-+-+-',0],2))
